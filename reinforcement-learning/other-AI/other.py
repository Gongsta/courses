...
class neuralNet:
	def __init__(self):
		...
		self.L1 = tf.reduce_sum(tf.square(self.Y1 - tf.multiply(self.o_fc21,self.C1)))
		self.L2 = tf.reduce_sum(tf.square(self.Y2 - tf.multiply(self.o_fc22,self.C2)))
		self.L3 = tf.reduce_sum(tf.square(self.Y3 - tf.multiply(self.o_fc23,self.C3)))
...
def process_image(ot):
	ot = np.array(Image.fromarray(ot).resize((N,N)))
